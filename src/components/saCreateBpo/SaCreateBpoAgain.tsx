...
import {
    ...
    getListBa,
    createBpo,
} from "api/createBpo/createBpoApi";
...
type SaCreateBpoAgain = {
    onCloseModal: () => void;
    togglePopupHistory: Function;
    listingId: any;
    listingTypeId: any;
    onHandlerPermission: (permission: boolean) => void;
};
const SaCreateBpoAgain: React.FC<SaCreateBpoAgain> = (props) => {
    const [permission, setPermission] = useState(false);
    const [message, setMessage] = useState<string | null>("");
    const { togglePopupHistory } = props;
    ...
    const [
        categoryConfig,
        setCategoryConfig,
    ] = useState<CategoryConfigResponse>({
    ...
    useEffect(() => {
        (async () => {
            //call api configure
            const responseConfigure = await getCategoryConfig({
                rListingId: props.listingId,
            });
...
export default SaCreateBpoAgain;
